...
use super::pool::Pool;
use crate::client::session::TlsSessionCache;
...
pub async fn run(cfg: ClientConfig) -> Result<()> {
    ...
    let cache = Arc::new(TlsSessionCache::new(4));
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(e) = handle(stream, cfg, pool, cache).await {
...
async fn handle(
    ...
    let (target, prefix): (Target, Vec<u8>) = if peek[0] == 0x05 {
        match super::socks5::handshake(&mut stream).await? {
            ...
            super::socks5::SocksTarget::UdpAssociate { .. } => {
                anyhow::bail!("UDP ASSOCIATE is not supported in this version");
            }
        }
    } else {
    ...
    let mut ws = acquire_tunnel(&cfg, &pool, cache, &req).await?;
...
async fn do_handshake(ws: &mut WsStream, req_text: &str) -> Result<()> {
    ...
    match msg {
        Message::Text(t) => {
            ...
            if !resp.ok {
                anyhow::bail!("server refused: {}", resp.err.unwrap_or_default());
            }
            Ok(())
        }
