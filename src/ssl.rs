...
use boring::{
    error::ErrorStack,
    ssl::{self, ErrorCode, ShutdownResult, Ssl, SslAcceptor as SslAcceptorCore, SslRef, SslStream as SslStreamCore},
};
...
pub struct SslAcceptor(SslAcceptorCore);

impl SslAcceptor {
    pub fn new(inner: SslAcceptorCore) -> Self {
        Self(inner)
    }

    pub async fn accept<S>(&self, stream: S) -> Result<SslStream<S>, ssl::Error>
    where
        S: AsyncRead + AsyncWrite,
    {
        let ssl = Ssl::new(self.0.context()).map_err(ssl::Error::from)?;
        let mut stream = SslStream::new(ssl, stream).map_err(ssl::Error::from)?;
        unsafe { Pin::new_unchecked(&mut stream) }.accept().await?;
        Ok(stream)
    }
}
